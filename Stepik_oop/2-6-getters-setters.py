...
Метод должен проверять, что в новой почте есть только один символ @
и после нее есть точка.
...
class UserMail:
    ''' Делает что-то хорошее. Содержит имя и почту '''
    ...
    def set_email(self, value):
        if isinstance(value, str) and re.match(r'\w+@\w+\.\w+\Z', value):
            self.__email = value
        else:
